...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, num_heads):
        ...
        #assert(self.head_dims * num_heads == embed_size, "embed_size/num_heads is not valid")
    ...
    def forward(self, keys, queries, values, mask=None):
        ...
        out = torch.einsum("nhqk,nvhd->nqhd", [energy, values])
...
if __name__ == "__main__":
    model = Encoder(1000, 256, 10, 8, "cpu", 4, 0.1, 50)
    ...
    embed = nn.Embedding(1000, 5)
    print(embed(torch.IntTensor([1,100,999, 5, 6, 7, 8, 9,10])).shape)
